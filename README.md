...
## 1. EC2에서 Docker build 시 서버 다운

* **원인**

  * EC2 (t2/t3 micro)에서 Gradle + Docker 빌드 시 리소스 부족

* **해결**

  * GitHub Actions에서 Docker 이미지 빌드 및 ECR push
  * ECR에 push 후 EC2는 pull만 수행하도록 구조 변경

---

## 2. Docker 환경에서 .env 미적용 문제

* **원인**

  * Docker는 `.env`를 자동으로 읽지 않음

* **해결**

  * `--env-file` 옵션으로 환경 변수 명시적 주입
...
* **원인**
  ...
  * 기존 jar 서비스(systemd)가 동일 포트 사용

* **해결**

  ```bash
  sudo systemctl stop paceleague
  sudo systemctl disable paceleague
  ```

---

## 4. Spring Boot 프로필 미적용

* **원인**

  * `application-local.yml`이 로드되지 않음
...
* 컨테이너 기반 배포 환경을 직접 구축하면서
  **“빌드와 실행 환경을 분리하는 구조”**의 중요성을 이해했습니다.
* EC2에서 직접 빌드하는 방식의 한계를 경험하고
  **ECR 기반 배포 구조로 개선**했습니다.
* 단순 개발을 넘어 **운영 환경까지 고려한 설계 경험**을 얻었습니다.
