...
- **`hex_count`(territory 테이블, 생성 시 고정)**: 매 상호작용마다 `COUNT(*)`을 다시 하지 않도록 겹침 비율 계산의 분모를 미리 저장해둔 값. 마이그레이션: `docs/migrations/2026-09-05_territory_hex_grid.sql`. **주의**: 이 전환 이전에 이미 생성된 ACTIVE territory는 `territory_hex` 행이 없어 배포 후 새 러닝과 상호작용(공격/회복/점령)하지 못하는 "유령 땅"이 된다 — 백필이 필요하면 마이그레이션 파일의 주석 참고(SQL만으로는 불가능, 애플리케이션 코드로 `polygon_json`을 헥사곤화해야 함).
