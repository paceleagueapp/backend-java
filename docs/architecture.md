...
- `board.domain.policy.BoardLabelPolicy` — slug(`free`/`qna`/`verify`/`crew_promo`) × `Language` 9개(한국어 제외) 고정 테이블. 한국어는 DB의 `board.name`/`board.description`이 이미 원본이므로 테이블에 중복 저장하지 않고, `lang=ko`거나 테이블에 없는 slug(신규 보드 추가 시)면 항상 DB 값을 그대로 반환한다.
...
- **폴리곤 계산에 JTS 도입**: `org.locationtech.jts:jts-core`(순수 Java, 전이 의존성 없음). 러닝 GPS 루프는 임의의 오목 다각형이라 면적/교집합을 직접 구현하면 버그 소지가 커서 도입. 공간 DB 타입(`GEOMETRY`)은 여전히 안 쓰고, 좌표는 `polygon_json`(LONGTEXT)에 저장하고 계산만 Java에서 한다(`record_track.points_json`과 동일 방침). `PolygonGeometry`가 위/경도를 bbox 중심 기준 등거리 평면(미터)으로 투영해 JTS로 계산. 닫힌 도형 여부·둘레·면적 검증(`ClosedLoopDetector`/`TerritoryClaimValidator`)은 지금도 이 실제 러닝 폴리곤 기준 그대로다.
...
**2026-09-05: 소유권/겹침 판정을 폴리곤 교집합에서 H3 헥사곤 격자(resolution 12)로 전환.** 계기는 "뛴 도형만큼만 내 땅"이 아니라 도형 안에 포함되거나 걸쳐진 헥사곤까지 전부 소유하게 해달라는 요청 — HP는 요청대로 territory 단위를 유지했다.

- **`com.uber:h3:4.5.0`**: JTS와 달리 순수 Java가 아니라 JNI + 플랫폼별 네이티브 바이너리를 내장한다(linux-x64 포함, Apache 2.0). 도형→헥사곤 변환은 `H3Core.polygonToCellsExperimental(..., PolygonToCellsFlags.containment_overlapping)` — 셀 중심이 안에 있는 것만 잡는 `containment_center`가 아니라 "도형에 조금이라도 걸치면 포함"을 쓴다. `H3Core`는 초기화가 네이티브 라이브러리를 디스크에 푸는 I/O 작업이라 `TerritoryH3Config`에서 싱글턴 빈으로 등록해 `ProcessTerritoryRunService`에 직접 주입한다(`StringRedisTemplate`과 같은 "포트화하지 않는 제네릭 인프라 클라이언트" 패턴).
- **검증(닫힌 도형/둘레/면적)과 소유권 표현을 분리.** 위 세 정책은 그대로 실제 러닝 폴리곤 기준으로 동작하고, 검증을 통과한 폴리곤만 `H3TerritoryGrid.coverRing`으로 헥사곤 집합으로 바뀐다. `PolygonGeometry.intersectionAreaSqm`(및 그 테스트)은 더 이상 이 흐름에서 호출되지 않지만, 독립적인 순수 지오메트리 유틸이자 기존 테스트가 있어 그대로 남겨뒀다.
- **새 테이블 `territory_hex`(`h3_index` PK, `territory_sno`, `season`)** — 헥사곤 1개가 어느 territory에 속하는지 매핑. `h3_index`가 PK라 "한 헥사곤은 동시에 하나의 ACTIVE territory에만 속한다"는 불변식을 DB 유니크 제약으로도 보장한다. **캡처(점령)가 일어나도 이 매핑은 절대 바뀌지 않는다** — HP/소유자는 여전히 `territory` 행 하나에만 있고(요청대로 territory 단위 HP 유지), 캡처는 지금까지처럼 그 행의 `owner_member_sno`만 바꾼다. 이 덕분에 캡처 로직 자체(`TerritoryDamagePolicy`)는 전혀 안 바꿨다 — 데미지/회복 비율 계산이 겹친 면적 대신 겹친 헥사곤 개수를 받을 뿐, 공식(`overlap/target` 비율 × maxHp × 계수)은 동일하다.
- **동시성**: 겹침 판정 순서가 "이번 러닝의 헥사곤 IN 리스트로 어느 territory와 겹치는지 집계(`territory_hex` ↔ `territory` 조인) → 그 territory 행들만 `findAllByIdForUpdate`로 `PESSIMISTIC_WRITE` 락"으로 바뀌었다. 기존 `findActiveIntersectingBboxForUpdate`(bbox 전체 후보를 락)는 제거 — 락 대상이 넓은 bbox 후보군에서 실제로 겹친 것으로 확인된 territory로 좁아졌다. 지도 뷰포트 조회(`findActiveIntersectingBbox`, 비잠금)는 그대로 bbox 기반이라 영향 없음. `GpsSessionSweeper`처럼 단일 인스턴스 전제는 유지 — 스케일아웃 시 재검토 필요.
- **지도에 그려지는 폴리곤이 바뀐다.** 새 땅 생성 시 `polygon_json`에는 이제 원본 러닝 경로가 아니라 소유 헥사곤 집합의 합집합 외곽선(`H3TerritoryGrid.unionBoundaryLatLng`, JTS `UnaryUnionOp`로 계산 후 위/경도로 역투영)이 저장된다 — territory 생성 시 1회만 계산해 저장하므로 지도 조회(`TerritoryQueryService.getMap`)는 무수정. `TerritoryMapResponse`/`TerritoryView` API 계약도 그대로다. `area_sqm`은 원본 러닝 면적이 아니라 헥사곤 합계 면적(`cellArea` 합)으로 바뀌어 랭킹(`SUM(area_sqm)`)이 실제 소유 면적을 반영한다. `perimeter_m`은 검증용 원본 러닝 둘레를 그대로 저장한다.
- **`hex_count`(territory 테이블, 생성 시 고정)**: 매 상호작용마다 `COUNT(*)`을 다시 하지 않도록 겹침 비율 계산의 분모를 미리 저장해둔 값. 마이그레이션: `docs/migrations/2026-09-05_territory_hex_grid.sql`. **주의**: 이 전환 이전에 이미 생성된 ACTIVE territory는 `territory_hex` 행이 없어 배포 후 새 러닝과 상호작용(공격/점령)하지 못하는 "유령 땅"이 된다 — 백필이 필요하면 마이그레이션 파일의 주석 참고(SQL만으로는 불가능, 애플리케이션 코드로 `polygon_json`을 헥사곤화해야 함).

**2026-09-05(같은 날, 후속): HP를 완전히 제거 — 겹치면 무조건 즉시 점령.** 사용자의 명시적 요청. 위 단락에서 "요청대로 territory 단위 HP 유지"라고 적었던 것과 달리, 이번 요청은 HP 자체를 없애고 "겹치면 뒤에 뛴 사람 것"으로 단순화하는 것이었다.

- **삭제**: `TerritoryDamagePolicy`(+ 테스트), `TerritoryContribution` 엔티티/포트/JPA 레포/어댑터(`territory_contribution` 테이블 전체), `Territory.hp`/`maxHp`/`applyDamage`/`heal`/`isDepleted`, `TerritoryProperties`의 `defaultMaxHp`/`attackFactor`/`healFactor`/`contributionWindowMinutes`. `Territory.capture`는 이제 `owner_member_sno`만 바꾸고 아무것도 리셋하지 않는다.
- **`ProcessTerritoryRunService.captureOverlapping`**: 겹친 territory마다 — 내 땅이면 스킵(그래도 "빈 땅이면 즉시 내 땅" 규칙을 위해 새 땅 생성은 막는다), 남의 땅이면 비율/임계값 계산 없이 바로 `target.capture(runner)`. 데미지 계산·기여도 조회·1시간 윈도우·동점 처리 로직이 전부 사라져 코드가 크게 단순해졌다.
