...
## 2026-09-17 (같은 날, 후속): 애드센스 "가치가 낮은 콘텐츠" 주의 조치

애드센스 심사에서 "가치가 낮은 콘텐츠" 주의를 받아 확인한 결과, `adsbygoogle.js` 로더 스크립트가 **14개 페이지 전부**(약관/로그인/회원가입 포함)에 로드되어 있었고, 실제 `<ins class="adsbygoogle">` 광고 유닛은 코드 어디에도 없어 **자동 광고(Auto ads)**에 전적으로 의존하는 구조임을 확인했습니다. 즉 콘텐츠가 거의 없는 약관·로그인·회원가입 페이지에도 구글이 알아서 광고를 꽂아 넣고 있었던 것이 주의 사유로 추정됩니다.

사용자 결정에 따라 **약관/개인정보/위치기반약관/계정삭제안내(ko+en 8개) + login.html + join.html, 총 10개 페이지에서 `adsbygoogle.js` 스크립트를 제거**했습니다. 실질 콘텐츠가 있는 `index.html`(커뮤니티 피드)·`post.html`(게시글)과, 보수적으로 유지하기로 한 `crew.html`·`territory.html`(앱 위젯성 페이지지만 함께 유지)에는 스크립트를 남겨뒀습니다.

**남은 과제**: 이건 광고 배치 문제만 해결한 것이고, `index.html`/`post.html`의 실제 게시글 콘텐츠 양·품질 자체가 부족하면 재심사에서 다시 "가치가 낮은 콘텐츠"로 걸릴 수 있습니다 — 실제 게시판에 유의미한 글이 충분히 쌓여 있는지는 이 저장소(코드)만으로는 확인할 수 없어 별도로 점검이 필요합니다.

## 2026-09-17 (같은 날, 세 번째 후속): 애드센스 재심사 전 `about.html` 소개 페이지 추가

애드센스 정책 센터에서 실제로 "가치가 낮은 콘텐츠" 위반이 발견된 상태였고(광고 스크립트 제거만으로는 부족), 사용자 확인 결과 게시판에 실제 게시글이 거의 없어 커뮤니티 콘텐츠만으로는 최소 콘텐츠 요건을 충족하기 어려운 상태였습니다.

가장 통제 가능한 대응으로 `web/about.html`(서비스 소개 페이지)을 새로 작성했습니다 — 러닝 기록/페이스·칼로리 계산, 랭킹·티어, 랜드잇(땅따먹기), 크루, 커뮤니티 각 기능을 실제 동작 방식 그대로 설명하는 고유 텍스트로 구성(섹션 6개, 회원가입/로그인 CTA 포함). `index.html` 하단에 "서비스 소개" 링크를 추가해 내부 링크로 연결했고, `sitemap.xml`에도 등록했습니다.

게시판 실제 콘텐츠(유의미한 게시글 수)가 늘어나는 것과는 별개 조치이므로, 재심사 요청 전 게시판 콘텐츠도 함께 점검하는 것을 권장합니다.

## 2026-09-17 (네 번째 후속): 사이트 내부 링크에서 `.html` 확장자 전부 제거

사용자 요청으로 주소창에 `.html`이 보이지 않도록 사이트 전체 내부 링크(네비게이션, `app.js`/`admin.js`의 `location.href` 리다이렉트, canonical 태그, `sitemap.xml`, 관리자 패널 내부 링크 포함)를 확장자 없는 형태로 통일했습니다. 실제 `.html` 파일은 디스크에 그대로 두었고(Nginx가 그 파일을 찾아야 하므로), 바뀐 건 링크/canonical/sitemap 문자열뿐입니다.

이게 가능했던 이유: 위 "서치 콘솔" 섹션에서 이미 확인한 대로 Nginx `try_files`가 사이트 전체에서 확장자 없는 URL도 200으로 서빙합니다(`/login`, `/ko/privacy`, `/admin/members` 등 전부 curl로 재확인). 즉 저장소(git) 파일만 고쳐서 "사이트 안에서 이동할 때 주소에 .html이 안 보이는" 목표를 달성했고, Nginx 설정은 건드리지 않았습니다.
