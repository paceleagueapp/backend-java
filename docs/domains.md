...
- 폴리곤 면적/둘레 검증(닫힌 도형 판정 포함)은 공간 DB 타입 대신 등거리 근사 평면 투영 + JTS로 Java에서 계산(`record_track.points_json`과 동일한 "좌표는 JSON, 계산은 Java" 방침). 수 km 규모에서 오차 무시 수준.
