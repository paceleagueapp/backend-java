...
public class BoardService implements BoardUseCase {
    ...
    public Long createPost(Long memberSno, Long boardSno, PostCreateRequest req) {
        ...
        if (req.attachmentMediaIds() != null && !req.attachmentMediaIds().isEmpty()) {
            mediaUseCase.attachToPost(memberSno, req.attachmentMediaIds(), post.getSno());
        }
    ...
    // 본인 소유 기록만 첨부할 수 있도록 검증(recordQueryUseCase.getOne은 memberSno+recordSno 조합으로만 조회됨).
    private void validateRecordOwnership(Long memberSno, Long recordSno) {
        if (recordSno != null) {
            recordQueryUseCase.getOne(memberSno, recordSno);
        }
    }
