...
class ProcessTerritoryRunServiceTest {
    ...
    void setUp() throws Exception {
        ...
        service = new ProcessTerritoryRunService(
                territoryRepositoryPort, territoryHexRepositoryPort, contributionRepositoryPort,
    ...
    private Territory existingTerritory(Long owner, int maxHp, int hexCount) {
    ...
    void 겹치는_땅이_없으면_새_땅을_생성한다() {
        ...
        verify(territoryHexRepositoryPort).saveAll(argThat(hexes ->
                hexes.size() == saved.getValue().getHexCount()));
    }

    @Test
    void 남의_땅과_겹치면_데미지를_주고_기여도를_남긴다() {
        Territory target = existingTerritory(200L, 1000, 50);
        when(territoryHexRepositoryPort.findActiveOverlapCounts(any()))
                .thenReturn(List.of(new TerritoryHexOverlap(null, 10L)));
    ...
    @Test
    void HP가_0이_되면_공격자가_점령하고_기여도를_비운다() {
        Territory target = existingTerritory(200L, 1, 50); // maxHp 1 → 한 번에 소진
        when(territoryHexRepositoryPort.findActiveOverlapCounts(any()))
                .thenReturn(List.of(new TerritoryHexOverlap(null, 10L)));
        when(territoryRepositoryPort.findAllByIdForUpdate(any()))
                .thenReturn(new ArrayList<>(List.of(target)));
    ...
    @Test
    void 내_땅과_겹치면_회복하고_기여도는_남기지_않는다() {
        Territory mine = existingTerritory(100L, 100, 50);
